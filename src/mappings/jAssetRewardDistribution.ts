...
import {
  JAssetMerkleTree,
  JAssetRewardDistributionSnapshot,
  UserJAssetRewardDistributionMetaData,
  UserClaimedJAssetRewardDistribution,
} from '../types/schema';
...
export function handleClaimed(event: ClaimedEvent): void {
  ...
  let userClaimedData = UserClaimedJAssetRewardDistribution.load(address.toHexString());
  if (userClaimedData == null) {
    userClaimedData = new UserClaimedJAssetRewardDistribution(address.toHexString());
...
export function handleEpochAdded(event: EpochAddedEvent): void {
  ...
  let snapshot = new JAssetRewardDistributionSnapshot(snapshotId.toString());
...
export function handleMetaData(content: Bytes): void {
  let jAssetMerkleTree = new JAssetMerkleTree(dataSource.stringParam());
  jAssetMerkleTree.merkleTree = content;
  jAssetMerkleTree.save();
  ...
  for (let index = 0; index < distributionsData.length; index++) {
    ...
    if (BigInt.fromString(distribution.amount) > new BigInt(0)) {
      ...
      let userRewardDistributionMetadata = new UserJAssetRewardDistributionMetaData(key);
